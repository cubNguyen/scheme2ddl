...
public class Main {
	...
	public static void main(String[] args) throws Exception {
		...
		if (justTestConnection) {
			...
			// todo remove this System.out.println and move this params to bean params, not
			// job params
			...
			new UserObjectJobRunner().start(context, isLaunchedByDBA, objectFilter.toLowerCase(),
					typeFilter.toUpperCase(), typeFilterMode.toLowerCase(), ddlTimeAfter, ddlTimeBefore, ddlTimeIn);
		}
	}
	...
	private static void modifyContext(ConfigurableApplicationContext context) {
		...
		if (ddlTimeAfter == null && ddlTimeFilterMap != null && ddlTimeFilterMap instanceof Map
				&& ddlTimeFilterMap.containsKey("ddlTimeAfter")) {
			String da = (String) ddlTimeFilterMap.get("ddlTimeAfter");
			if (da != null)
				ddlTimeAfter = da;
		}
		if (ddlTimeBefore == null && ddlTimeFilterMap != null && ddlTimeFilterMap instanceof Map
				&& ddlTimeFilterMap.containsKey("ddlTimeBefore")) {
			String df = (String) ddlTimeFilterMap.get("ddlTimeBefore");
			if (df != null)
				ddlTimeBefore = df;
		}
		if (ddlTimeIn == null && ddlTimeFilterMap != null && ddlTimeFilterMap instanceof Map
				&& ddlTimeFilterMap.containsKey("ddlTimeIn")) {
		...
		FileNameConstructor fileNameConstructor = retrieveFileNameConstructor(context); // will create new one if not
																						// exist
		if (isLaunchedByDBA) {
		...
		if (schemas == null) {
			if (listFromContext.size() == 0) {
				...
				if (isLaunchedByDBA)
					...
					log.warn(
							"Ignore 'schemaList' from advanced config, because oracle user is not connected as sys dba");
					schemaList = extactSchemaListFromUserName(context);
				}
			}
		} else {
	...
	/**
	 * @param context
	 * @return existing bean 'schemaList', if this exists, or create and register
	 *         new bean
	 */
	private static List<String> retrieveSchemaListFromContext(ConfigurableApplicationContext context) {
		...
		try {
			...
			beanFactory.registerBeanDefinition("schemaList",
					BeanDefinitionBuilder.rootBeanDefinition(ArrayList.class).getBeanDefinition());
			list = (List) context.getBean("schemaList");
		}
		return list;
	}
	...
	/**
	 * @param context
	 * @return existing bean 'fileNameConstructor', if this exists, or create and
	 *         register new bean
	 */
	private static FileNameConstructor retrieveFileNameConstructor(ConfigurableApplicationContext context) {
		...
		try {
			...
			beanFactory.registerBeanDefinition("fileNameConstructor",
					BeanDefinitionBuilder.rootBeanDefinition(FileNameConstructor.class).getBeanDefinition());
			fileNameConstructor = (FileNameConstructor) context.getBean("fileNameConstructor");
			fileNameConstructor.afterPropertiesSet();
	...
	private static void validateContext(ConfigurableApplicationContext context) {
		...
		if (!isLaunchedByDBA) {
			...
			Assert.state(userName.startsWith(schemaName),
					String.format("Cannot process schema '%s' with oracle user '%s', if it's not connected as sysdba",
	...
	private static void printUsage() {
		...
		msg.append("                             example: 2016-11-27 : exports objects that modified after 27 Nov 2016"
				+ lSep);
		msg.append(
		...
		msg.append("                             example: \"6 hour\" : exports objects that modified in last 6 hours"
				+ lSep);
	...
	private static void collectArgs(String[] args) throws Exception {
		...
		for (int i = 0; i < args.length; i++) {
			...
			if (arg.equals("-help") || arg.equals("-h") || arg.equals("--help")) {
				...
				if (args.length >= i + 3) {
					if ((args[i + 2].toLowerCase().equals("as")) && (args[i + 3].toLowerCase().startsWith("sysdba"))) {
				...
				if (objectFilter.length() > 0 && objectFilter.charAt(objectFilter.length() - 1) == '\''

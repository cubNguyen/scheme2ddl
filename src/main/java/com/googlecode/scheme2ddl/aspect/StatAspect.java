...
/**
 * Collect statistics about skipped objects and print detailed report after end
...
public class StatAspect {
	...
	@AfterReturning(pointcut = "execution(* com.googlecode.scheme2ddl.UserObjectProcessor.process(..)) &&"
			+ "args(userObject)", returning = "retVal")
	...
	@After("execution(* org.springframework.batch.core.launch.JobLauncher.run(..)) &&" + " args(job, jobParameters)")
	public void printStatistic(Job job, JobParameters jobParameters) {
		...
		// System.out.println("listExludedByConfig = " + listExludedByConfig); //todo
		// pretty print
		// System.out.println("listSkippedBySQLError = " + listSkippedBySQLError);
		// //todo pretty print
	}
	...
	private void prettyPrintList(StringBuilder sb, String ruleName, List<UserObject> listExludedByConfig) {
		...
		for (String type : groupByType.keySet()) {
			...
			sb.append(
					formatter.format("| %s |  %-24s |      %-6s |", ruleName, type, groupByType.get(type)).toString());
			sb.append(lSep);
		}
	}
